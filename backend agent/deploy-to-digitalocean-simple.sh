...
# ViWorks Backend Agent - Simple Digital Ocean Deployment Script
# This script deploys the pre-built backend agent binary to the remote server

set -e

echo "🚀 Starting ViWorks Backend Agent Deployment to Digital Ocean (Simple Method)..."
...
print_step "5. Creating and starting container with pre-built binary..."
ssh ${REMOTE_USER}@${REMOTE_HOST} "
    cd ${REMOTE_DIR}
    ...
    docker exec ${CONTAINER_NAME} bash -c '
        ...
        cd /app
        chmod +x viworks-backend-agent
    '
    
    echo 'Starting the application...'
    docker exec -d ${CONTAINER_NAME} bash -c '
        cd /app
        nohup ./viworks-backend-agent > logs/app.log 2>&1 &
    '
"
