...
impl TelemetryStorage {
    ...
    pub async fn store_telemetry(&self, telemetry: &TelemetryRecord) -> BackendAgentResult<()> {
        ...
        self.data_layer.redis.set(&cache_key, &telemetry_json, Some(std::time::Duration::from_secs(3600))).await?;

        debug!("Telemetry stored successfully for agent: {}", telemetry.agent_id);
        Ok(())
    }
    ...
    pub async fn get_latest_telemetry(&self, agent_id: &str) -> BackendAgentResult<Option<TelemetryRecord>> {
        ...
        let telemetry = self.data_layer.postgres.get_latest_telemetry(agent_id).await?;
        ...
        if let Some(ref telemetry_data) = telemetry {
            ...
            if let Err(e) = self.data_layer.redis.set(&cache_key, &telemetry_json, Some(std::time::Duration::from_secs(3600))).await {
    ...
    pub async fn get_telemetry_history(&self, agent_id: &str, limit: Option<usize>) -> BackendAgentResult<Vec<TelemetryRecord>> {
        debug!("Getting telemetry history for agent: {} (limit: {:?})", agent_id, limit);
        ...
        match self.data_layer.postgres.get_latest_telemetry(agent_id).await? {
            Some(latest) => {
        ...
        debug!("Retrieved {} telemetry records for agent: {}", telemetry_records.len(), agent_id);
        Ok(telemetry_records)
    }

    /// Get telemetry for multiple agents
    pub async fn get_telemetry_for_agents(&self, agent_ids: &[String]) -> BackendAgentResult<Vec<TelemetryRecord>> {
        ...
        debug!("Retrieved telemetry for {} out of {} agents", all_telemetry.len(), agent_ids.len());
        Ok(all_telemetry)
    }

    /// Clean up old telemetry data
    pub async fn cleanup_old_telemetry(&self, threshold_hours: i64) -> BackendAgentResult<usize> {
        info!("Cleaning up telemetry data older than {} hours", threshold_hours);
    ...
    pub async fn batch_store_telemetry(&self, telemetry_batch: Vec<TelemetryRecord>) -> BackendAgentResult<usize> {
        ...
        for telemetry in telemetry_batch {
            if let Err(e) = self.store_telemetry(&telemetry).await {
                error!("Failed to store telemetry for agent {}: {}", telemetry.agent_id, e);
            } else {
                successful_count += 1;
            }
        }
    ...
    pub async fn get_telemetry_summary(&self, agent_id: &str) -> BackendAgentResult<Option<TelemetrySummary>> {
        ...
        if let Some(latest) = self.get_latest_telemetry(agent_id).await? {
            let summary = TelemetrySummary {
                ...
                memory_usage: latest.memory_usage.used_mb as f64 / latest.memory_usage.total_mb as f64 * 100.0,
                disk_usage: latest.disk_usage.iter()
                    .map(|d| d.usage_percent)
                    .fold(0.0, |acc, x| acc + x) / latest.disk_usage.len() as f64,
                network_bytes: latest.network_stats.as_ref()
                    .map(|n| n.bytes_received)
                    .unwrap_or(0),
                network_packets: latest.network_stats.as_ref()
                    .map(|n| n.bytes_sent)
                    .unwrap_or(0),
