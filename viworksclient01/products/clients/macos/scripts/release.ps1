...
# This script creates a complete production release with all necessary checks
...
Usage: .\release.ps1 [options]

Options:
    -Version <version>  Specify version number (default: 1.0.0)
    -SkipTests         Skip running tests
    -SkipAudit         Skip security audit
    -SkipDeny          Skip dependency analysis
    -Help              Show this help message

Examples:
    .\release.ps1                    # Full release with all checks
    .\release.ps1 -Version 1.1.0     # Release with specific version
    .\release.ps1 -SkipTests         # Release without tests
"@
    exit 0
}
...
function Invoke-CleanBuild {
    ...
    # Remove Cargo.lock (will be regenerated)
...
function Invoke-DependencyAnalysis {
    if ($SkipDeny) {
        Write-ColorOutput "Skipping dependency analysis as requested" $Yellow
        return
    }
    ...
    try {
        ...
        Write-ColorOutput "Installing cargo-deny..." $Yellow
        cargo install cargo-deny
    }
    
    $result = cargo deny check
    
    if ($LASTEXITCODE -eq 0) {
        Write-ColorOutput "✓ Dependency analysis passed" $Green
    }
    else {
        Write-ColorOutput "✗ Dependency analysis found issues" $Red
        Write-ColorOutput "Please review and fix the dependency issues before proceeding" $Yellow
        exit 1
    }
}

function Invoke-SecurityChecks {
    Write-Header "Additional Security Checks"
    
    # Check for hardcoded secrets
    $secrets = Get-ChildItem -Recurse -Include "*.rs", "*.toml", "*.json" | 
               Select-String -Pattern "(password|secret|key|token).*=.*['`"][^'`"]+['`"]" |
               Where-Object { $_.Line -notmatch "//.*" -and $_.Line -notmatch "#.*" }
    
    if ($secrets) {
        Write-ColorOutput "⚠ Potential hardcoded secrets found:" $Yellow
        $secrets | ForEach-Object {
            Write-ColorOutput "  $($_.Filename):$($_.LineNumber): $($_.Line.Trim())" $Yellow
        }
    }
    else {
        Write-ColorOutput "✓ No hardcoded secrets found" $Green
    }
    
    # Check for unsafe code usage
    $unsafeCount = (Get-ChildItem -Recurse -Include "*.rs" | 
                   Select-String -Pattern "unsafe" | 
                   Measure-Object).Count
    
    Write-ColorOutput "✓ Found $unsafeCount unsafe code blocks" $Green
    
    # Check for proper error handling
    $unwrappedCount = (Get-ChildItem -Recurse -Include "*.rs" | 
                      Select-String -Pattern "\.unwrap\(\)|\.expect\(" | 
                      Measure-Object).Count
    
    Write-ColorOutput "Found $unwrappedCount unwrap/expect calls" $Green
}
...
function Invoke-CreateDistribution {
    ...
    # Copy runtime dependencies to the same directory as the executable for testing
    Write-ColorOutput "Copying runtime dependencies for testing..." $Blue
    if (Test-Path "binaries") {
        Copy-Item "binaries\*" "$releaseDir\" -Force
        Write-ColorOutput "  ✓ Copied all runtime dependencies" $Green
    } else {
        Write-ColorOutput "  ✗ Binaries folder not found. Please ensure binaries are in the correct location." $Red
        exit 1
    }
...
function Invoke-CreateReleaseNotes {
...
# Main execution
try {
    Write-ColorOutput "ViWorkS Client Production Release Script" $Blue
