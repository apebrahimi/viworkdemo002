...
echo "🧹 Server Cleanup and Health Monitoring"
echo "======================================="
...
# Create the cleanup and monitoring script
cat > server_cleanup_script.sh << 'EOF'
#!/bin/bash
set -e
...
echo "=========================="
docker ps --format "table {{.Names}}\t{{.Names}}\t{{.Status}}\t{{.Ports}}\t{{.Image}}"
...
echo "📤 Uploading cleanup script to server..."
scp -i ~/.ssh/id_ed25519 server_cleanup_script.sh root@64.227.46.188:/tmp/

echo "🔧 Making script executable and running it..."
ssh -i ~/.ssh/id_ed25519 root@64.227.46.188 "chmod +x /tmp/server_cleanup_script.sh && /tmp/server_cleanup_script.sh"

# Clean up
rm -f server_cleanup_script.sh

echo ""
echo "✅ Server cleanup and monitoring completed!"
