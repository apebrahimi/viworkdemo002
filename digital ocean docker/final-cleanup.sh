...
# Create the final cleanup script
cat > final_cleanup_script.sh << 'EOF'
#!/bin/bash
set -e
...
echo "📤 Uploading final cleanup script to server..."
scp -i ~/.ssh/id_ed25519 final_cleanup_script.sh root@64.227.46.188:/tmp/

echo "🔧 Making script executable and running it..."
ssh -i ~/.ssh/id_ed25519 root@64.227.46.188 "chmod +x /tmp/final_cleanup_script.sh && /tmp/final_cleanup_script.sh"

# Clean up
rm -f final_cleanup_script.sh

echo ""
echo "✅ Final cleanup and setup completed!"
