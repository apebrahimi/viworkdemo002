...
const UsersSection = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'inactive':
        return <Badge variant="destructive" className="bg-red-100 text-red-800 border-red-200">Inactive</Badge>;
      default:
  ...
  const getDeviceStatusBadge = (status: string) => {
    switch (status) {
      case 'approved':
        return <Badge variant="success" className="bg-green-100 text-green-800 border-green-200">Approved</Badge>;
      case 'pending':
        return <Badge variant="warning" className="bg-yellow-100 text-yellow-800 border-yellow-200">Pending</Badge>;
      case 'rejected':
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-300 mt-1">
            Manage user accounts, roles, and device bindings.
          </p>
        </div>
        <div className="flex gap-3">
          ...
          >
            Device Requests ({deviceRequests.filter(r => r.status === 'pending').length})
          </Button>
          <Button 
      ...
      <Card className="bg-white dark:bg-gray-800 shadow-lg border border-gray-200 dark:border-gray-700">
        <div className="p-6">
          <h3 className="text-lg font-semibold mb-6 text-gray-900 dark:text-white">Users</h3>
                ...
                <div key={user.id} className="flex items-center justify-between p-6 border border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-500 dark:text-gray-400 text-xs">Mobile: {user.mobile}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-4">
      ...
      >
        <div className="space-y-6 p-6">
          ...
          <div>
            <label className="block text-sm font-semibold mb-2 text-gray-700 dark:text-gray-300">Mobile</label>
            <Input
          ...
          <div>
            <label className="block text-sm font-semibold mb-2 text-gray-700 dark:text-gray-300">Policy Window</label>
            <Input
      ...
      <Modal
        ...
        title="Device Binding Requests"
      >
        <div className="space-y-4 p-6">
          {deviceRequests.length === 0 ? (
            <div className="text-center py-12 text-gray-500 dark:text-gray-400">
              <div className="text-4xl mb-4">📱</div>
              <p className="text-lg font-medium">No device binding requests</p>
